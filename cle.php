...
<br><br>
 ...
 <table border= "1px">
 	...
 	<?php
				...
				$sql = "SELECT * FROM newstaff WHERE dep ='Claire Estate'"; //tukar kt categories kalau nk lunch, dinner, etc...
				$result = mysqli_query($conn,$sql);
				...
				if(mysqli_num_rows($result)>0){
					...
					while($row = mysqli_fetch_assoc($result)){
                        ...
                        $image_src = "image/".$image;
							...
							echo"<td><center><a href=\"clev.php?ID=".$row["s_id"]."\" class = btn1>VIEW MORE</a></center></td>";
							echo "</tr>";
